...
const flattenGroupChildren = (nodes, items = []) => {
  for (const node of nodes) {
    ...
    if (node.type === 'dir' && node.indexPath) {
      items.push({
        type: 'file',
        name: node.name,
        label: createDocLabel(node.name, 'group'),
        path: node.indexPath,
      });
    }

    if (Array.isArray(node.children) && node.children.length > 0) {
      flattenGroupChildren(node.children, items);
    }
  }
...
const getDocsTree = (dir, prefix = '') => {
  ...
  for (const entry of entries) {
    ...
    if (entry.isDirectory()) {
      ...
      const indexEntry = childTree.find((child) => child.type === 'dir-index') || null;
      docs.push({
        type: 'dir',
        name: entry.name,
        ...
        children: flattenGroupChildren(
          childTree.filter((child) => child.type !== 'dir-index'),
