...
const getMarkdownFiles = (dir, prefix = '', recursive = true) => {
  ...
  for (const entry of entries) {
    ...
    if (!prefix && entry.isFile() && entry.name === 'AGENTS.md') continue;
...
const compareDocEntries = (a, b) => {
  const topLabels = new Set(['overview', 'intro', 'introduction']);
  const aIsIntro = topLabels.has(String(a.label || '').toLowerCase());
  const bIsIntro = topLabels.has(String(b.label || '').toLowerCase());
...
const createDocLabel = (name, parentPath = '') => {
  const baseName = name.replace(/\.md$/i, '');
  const label = baseName
    .replace(/[-_]+/g, ' ')
    .replace(/\b\w/g, (char) => char.toUpperCase());

  if (['Introduction', 'Overview', 'Intro'].includes(label)) {
    return parentPath ? 'Overview' : 'Intro';
  }

  return label;
};

const getDocsTree = (dir, prefix = '') => {
  ...
  const rootReadmeEntry = !prefix
    ? entries.find((entry) => entry.isFile() && entry.name.toLowerCase() === 'readme.md')
    : null;

  if (rootReadmeEntry) {
    docs.push({
      type: 'file',
      name: rootReadmeEntry.name,
      label: 'Intro',
      path: prefix,
    });
  }
  ...
  for (const entry of entries) {
    if (entry.name.startsWith('.') || entry.name === 'node_modules') continue;
    if (!prefix && entry.isFile() && entry.name.toLowerCase() === 'readme.md') continue;
    if (!prefix && entry.isFile() && entry.name === 'AGENTS.md') continue;

    const entryPath = prefix ? path.join(prefix, entry.name) : entry.name;
    const fullPath = path.join(dir, entry.name);

    if (entry.isDirectory()) {
      const childTree = getDocsTree(fullPath, entryPath);
      const indexEntry = childTree.find(
        (child) =>
          child.type === 'file' &&
          path.parse(child.name).name.toLowerCase() === entry.name.toLowerCase(),
      ) || null;
      const children = childTree.filter((child) => child !== indexEntry);
      if (!indexEntry && children.length === 0) continue;

      docs.push({
        type: 'dir',
        name: entry.name,
        label: createDocLabel(entry.name, prefix),
        path: entryPath,
        indexPath: indexEntry ? indexEntry.path : null,
        children,
      });
      continue;
    }
  ...
  return docs.sort(compareDocEntries);
};

const flattenDocsTree = (tree, items = []) => {
  for (const node of tree) {
    if (node.type === 'file') {
      items.push({
        path: normalizeDocPath(node.path),
        label: node.label,
      });
      continue;
    }

    if (node.type === 'dir' && node.indexPath) {
      items.push({
        path: normalizeDocPath(node.indexPath),
        label: node.label,
      });
    }

    if (Array.isArray(node.children) && node.children.length > 0) {
      flattenDocsTree(node.children, items);
    }
  }
