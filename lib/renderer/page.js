...
const html = (
  ...
  const themePresets = ['notebook', 'ops', 'blueprint', 'editorial', 'corporate'];
  ...
  const docsTreeHtml = renderDocsTree(projectId, docsTree, currentDocPath);
  const docPaginationHtml = renderDocPagination(projectId, currentDocPath, flatDocs);
  ...
  const projectControlsHtml = `
      <div class="sidebar-panel project-controls-card">
        ...
        <div class="project-dropdown-row">
          ...
          <div class="project-dropdown-actions">
            <button class="project-manage-button" onclick="renameCurrentProject()" title="Rename project">Rename</button>
            <button class="project-manage-button" onclick="deleteCurrentProject()" title="Delete project">Remove</button>
            <button class="project-manage-button" onclick="showAddProject()" title="Add project">Add</button>
          </div>
        </div>
      </div>
    `;
  ...
  const sidebarBodyHtml = activeProject
    ? `
      ${projectControlsHtml}
      ${favoritesHtml}
      <div class="sidebar-panel docs-nav-panel">
        <div class="sidebar-panel-label">${docsTreeHtml ? 'Documentation' : 'No docs yet'}</div>
...
<body class="theme-${initialThemePreset}">
  ...
  <div class="app-shell">
    <header class="shell-header content-header">
      <div class="shell-header-inner">
        <div class="shell-header-sidebar">
        ...
        <div class="shell-header-main docs-main-container">
        <div class="header-nav">
          ...
          <div class="header-actions">
            ...
            <div class="page-actions header-page-actions" id="page-actions">
              <button id="content-only-button" class="page-action-button" onclick="toggleContentOnlyMode()" title="Show content only" aria-label="Show content only">
            ...
            ${filePath ? `<div class="editor-nav-actions" id="editor-nav-actions">
              ...
              <button type="button" class="editor-nav-button editor-nav-save" onclick="saveFile()">Save</button>
            </div>` : ''}
            ...
            <button id="density-toggle" class="density-toggle hidden-control" title="Toggle reading density">Compact</button>
          </div>
        </div>
        </div>
      </div>
  ...
  <script>
    ...
    const DENSITY_STORAGE_KEY = 'mermaid_density_mode';
    ...
    function sanitizeThemePreset(theme) {
      return ['notebook', 'ops', 'blueprint', 'editorial', 'corporate'].includes(theme)
        ? theme
        : 'notebook';
    }
    ...
    function getMermaidThemeConfig(theme) {
      const byPreset = {
        ...
        blueprint: {
          primaryColor: '#dff3f0',
          primaryTextColor: '#0f3d3a',
          primaryBorderColor: '#0f766e',
          secondaryColor: '#e7f7f4',
          secondaryTextColor: '#145f59',
          tertiaryColor: '#f2fbfa',
          lineColor: '#0f766e',
          clusterBkg: '#e2f2ef',
          clusterBorder: '#2ea69a',
          edgeLabelBackground: '#e9f7f5',
          mainBkg: '#f4f8fb',
          fontSize: '14px',
        },
        editorial: {
          primaryColor: '#efe5d7',
          primaryTextColor: '#2f241c',
          primaryBorderColor: '#6c4e37',
          secondaryColor: '#f7f1e8',
          secondaryTextColor: '#4e3a2c',
          tertiaryColor: '#fcfaf5',
          lineColor: '#7b5d47',
          clusterBkg: '#f2ebdf',
          clusterBorder: '#b39a83',
          edgeLabelBackground: '#f7f1e8',
          mainBkg: '#fffdf9',
          fontSize: '14px',
        },
        corporate: {
      ...
      return {
        ...
        themeVariables: byPreset[sanitizeThemePreset(theme)] || byPreset.notebook,
      };
    }
    ...
    function applyDensityMode(mode) {
      ...
      document.body.classList.toggle('compact-density', useCompact);
      if (densityToggleButton) {
        densityToggleButton.textContent = useCompact ? 'Comfort' : 'Compact';
        densityToggleButton.setAttribute(
          'title',
          useCompact ? 'Switch to comfortable spacing' : 'Switch to compact spacing',
        );
      }
    }
    ...
    function toggleDensityMode() {
      const nextMode = document.body.classList.contains('compact-density')
        ? 'comfortable'
        : 'compact';
    ...
    function setSidebarOpen(open) {
      if (!sidebarEl) return;
      const shouldOpen = typeof open === 'boolean' ? open : sidebarEl.classList.contains('hidden');
      sidebarEl.classList.toggle('hidden', !shouldOpen);
      document.body.classList.toggle('sidebar-open', shouldOpen && isMobileSidebar());
      fetch('/api/settings', {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ sidebarSticky: shouldOpen })
      });
    }
    ...
    function generateTOC() {
      ...
      tocSidebarEl.innerHTML = '<div class="toc-title">On this page</div><ul class="toc-list"></ul>';
    ...
    if (densityToggleButton) densityToggleButton.addEventListener('click', toggleDensityMode);
    if (sidebarContentEl) {
      sidebarContentEl.addEventListener('scroll', persistSidebarScroll, { passive: true });
    }
    if (sidebarEl && isMobileSidebar() && !sidebarEl.classList.contains('hidden')) {
