...
<body class="theme-${initialThemePreset}">
  ...
  <script>
    ...
    const NAV_STATE_STORAGE_KEY = 'mermaid_docs_nav_state:' + (projectId || 'dashboard');
    ...
    function revealActiveDocsPage(focusActivePage = false) {
      const activePageLink = document.querySelector('.docs-nav-panel [aria-current="page"]');
      if (!activePageLink || activePageLink.closest('[hidden], [inert]')) return false;
      requestAnimationFrame(() => {
        if (focusActivePage) activePageLink.focus({ preventScroll: true });
        activePageLink.scrollIntoView({ block: isMobileSidebar() ? 'center' : 'nearest' });
      });
      return true;
    }
    ...
    function toggleMobileNav(forceState) {
      ...
      if (shouldOpen) {
        requestAnimationFrame(() => {
          if (!revealActiveDocsPage(true)) focusMobileDrawerStart();
        });
      } else {
    ...
    function getDocsNavState() {
      try {
        return JSON.parse(sessionStorage.getItem(NAV_STATE_STORAGE_KEY) || '{}');
      } catch (e) {
        return {};
      }
    }

    function saveDocsNavState(state) {
      try {
    ...
    function bindDocsNavigation() {
      ...
      const setBranchOpen = (branch, isOpen) => {
        const childContainer = branch.querySelector(':scope > .docs-nav-children');
        const row = branch.querySelector(':scope > .docs-nav-row');
        branch.classList.toggle('is-open', isOpen);
        if (childContainer) childContainer.hidden = !isOpen;
        row?.querySelectorAll('[data-doc-toggle]').forEach((toggle) => {
          toggle.setAttribute('aria-expanded', isOpen ? 'true' : 'false');
          if (toggle.classList.contains('docs-nav-toggle')) {
            const label = toggle.getAttribute('aria-label') || '';
            toggle.setAttribute(
              'aria-label',
              label.replace(/^(Expand|Collapse)/, isOpen ? 'Collapse' : 'Expand'),
            );
          }
        });
      };

      const navState = getDocsNavState();
      document.querySelectorAll('[data-doc-branch]').forEach((branch) => {
        const row = branch.querySelector(':scope > .docs-nav-row');
        const isCurrentPath = row?.classList.contains('is-current-ancestor') ||
          !!row?.querySelector('[aria-current="page"]');
        const path = branch.dataset.path || '';
        if (isCurrentPath) {
          setBranchOpen(branch, true);
        } else if (path in navState) {
          setBranchOpen(branch, !!navState[path]);
        }
      });

      navRoot.addEventListener('click', (event) => {
        const showAllButton = event.target.closest('[data-doc-show-all]');
        if (showAllButton) {
          const filter = showAllButton.closest('[data-doc-filter]');
          const list = filter?.nextElementSibling;
          if (!list) return;
          list.dataset.showAll = 'true';
          list.querySelectorAll(':scope > [data-doc-overflow]').forEach((item) => {
            item.hidden = false;
          });
        ...
        const branchToggle = event.target.closest('[data-doc-toggle]');
        if (branchToggle) {
          const branch = branchToggle.closest('[data-doc-branch]');
          if (!branch) return;
          const isOpen = !branch.classList.contains('is-open');
          setBranchOpen(branch, isOpen);
          const state = getDocsNavState();
          state[branch.dataset.path || ''] = isOpen;
          saveDocsNavState(state);
          return;
        }

        if (isMobileSidebar()) {
          const link = event.target.closest('a.docs-nav-link, .project-link, .favorite-link');
          if (link) {
            closeMobileNav();
          }
        }
      ...
      navRoot.addEventListener('input', (event) => {
        const input = event.target.closest('.docs-nav-filter-input');
        if (!input) return;
        const filter = input.closest('[data-doc-filter]');
        const list = filter?.nextElementSibling;
        if (!list) return;
        ...
        let matchCount = 0;
        list.querySelectorAll(':scope > .docs-nav-item').forEach((item) => {
          const matches = !query || (item.dataset.docNavLabel || '').includes(query);
          if (matches) matchCount += 1;
          item.hidden = !matches || (!query && !showAll && item.dataset.docOverflow === 'true');
        });

        const showAllButton = filter.querySelector('[data-doc-show-all]');
        if (showAllButton) showAllButton.hidden = !!query || showAll;
        const status = filter.querySelector('[data-doc-filter-status]');
        if (status) status.textContent = query ? matchCount + ' matching items' : '';
      });

      restoreSidebarScroll();
      revealActiveDocsPage();
    }
    ...
    if (projectSwitcher) {
      projectSwitcher.addEventListener('change', (event) => {
        ...
        try {
          sessionStorage.removeItem(NAV_STATE_STORAGE_KEY);
        } catch (e) {
          // Ignore storage errors
        }
