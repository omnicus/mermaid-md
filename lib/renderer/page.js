...
<body class="theme-${initialThemePreset}">
  ...
  <script>
    ...
    function summarizeGitDiffRows(baseContent, currentContent) {
      return buildSideBySideDiffRows(baseContent, currentContent).reduce(
        (summary, row) => {
          if (row.type === 'added') {
            summary.added += 1;
            summary.changed += 1;
    ...
    function renderGitDiffSnapshot(state) {
      ...
      const summaryCounts = summarizeGitDiffRows(state.baseContent, state.currentContent);
      ...
      const html = sourceRows.length
        ? sourceRows
          .map((row) => {
            if (row.type === 'omitted') {
              return '<div class="git-diff-omitted-row"><div class="git-diff-omitted-cell">'
                + escapeHtmlClient('...' + row.skipped + ' unchanged line' + (row.skipped === 1 ? '' : 's') + '...')
                + '</div></div>';
            }
            return '<div class="git-diff-row">'
              + renderDiffCell('base', row)
              + renderDiffCell('current', row)
              + '</div>';
          })
          .join('')
        : '<div class="git-diff-empty-state">No git changes for this file.</div>';
