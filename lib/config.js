...
const normalizeSettings = (settings) => {
  ...
  if (
    !['notebook', 'ops', 'blueprint', 'editorial', 'corporate'].includes(
      next.themePreset,
    )
  ) {
    next.themePreset = DEFAULT_SETTINGS.themePreset;
  }
  if (!['system', 'light', 'dark'].includes(next.themeMode)) {
    next.themeMode = DEFAULT_SETTINGS.themeMode;
  }
  return next;
};
...
const updateSettings = (data) => {
  config.settings = normalizeSettings({ ...config.settings, ...data });
  saveConfig();
